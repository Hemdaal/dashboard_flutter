...
class AddSoftwareWidget extends StatelessWidget {
  ...
  AddSoftwareWidget(CreateSoftwareInfo softwareInfo, ValueChanged onValueChanged)
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(
          children: [
            TextFormField(
              onChanged: (name) => {_softwareInfo.name = name, _onValueChanged(_softwareInfo)},
              initialValue: _softwareInfo.name,
              decoration: InputDecoration(hintText: 'Software Name', border: OutlineInputBorder()),
            ),
  ...
  Widget _getCodeManagementWidget() {
    ...
    columns.add(Row(children: [
      Checkbox(
          ...
          onChanged: (enabled) => {
                if (enabled == true)
                  {_softwareInfo.enableCodeManagement(), _onValueChanged(_softwareInfo)}
                else
                  {_softwareInfo.disableCodeManagement(), _onValueChanged(_softwareInfo)}
              }),
  ...
  Widget _createCodeMangementWidget() {
    return Column(
      children: [
        ...
        TextFormField(
          onChanged: (url) => {_softwareInfo.getCodeManagementInfo()?.gitUrl = url, _onValueChanged(_softwareInfo)},
          initialValue: _softwareInfo.getCodeManagementInfo()?.gitUrl,
          decoration: InputDecoration(hintText: 'URL', border: OutlineInputBorder()),
        ),
        TextFormField(
          ...
          decoration: InputDecoration(hintText: 'Token', border: OutlineInputBorder()),
        ),
      ],
    );
  }
